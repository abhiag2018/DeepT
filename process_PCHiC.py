...
def select_promoters(pchicDF, split_col = 'baitPr',  select_ = 'random'):
    assert select_ in ['random','weight']

    def dupl_merge(df,split_col,copy_col=None):
        if copy_col is None:
            copy_col = df.columns
            copy_col = copy_col.remove(split_col)

        max_elem = df[split_col].apply(len).max()

        for i in range(max_elem):
            df[split_col+str(i)] = df[split_col].apply(lambda x:x[i] if len(x)>i else np.nan)

        df_list = []
        for i in range(max_elem):
            col_i = split_col+str(i)
            df_list.append(df[pd.notna(df[col_i])][copy_col+[col_i]].rename(columns = {col_i:split_col}))

        return pd.concat(df_list).sort_index()

    
    if select_=='random':
        pchicDF[split_col] = pchicDF[split_col].apply(lambda x:x[random.choice(np.arange(len(x)))])
        return pchicDF
    else:
        pchicDF['weight'] = pchicDF[split_col].apply(lambda x:1.0/len(x))
    return dupl_merge(pchicDF, split_col, copy_col=None)


def hicTrainingCSV(hicUniquePE, hicUniqueEP, prwin, enhwin, unique_interaction = "random", traindata_out = None):
    """
    select the data from the columns of the training data and put them in appropriate format
    """
    def niceHiCDF(pickle_path, prwin, enhwin, prefixPr, prefixEnh, unique_interaction):
        ...
        pchicDF['baitPr'] = pchicDF['baitPr'].apply(np.array)
        pchicDF['oePr'] = pchicDF['oePr'].apply(np.array)
        pchicDF = select_promoters(pchicDF, split_col = prefixPr+'Pr', select_ = unique_interaction)
    ...
    dfPE = niceHiCDF(hicUniquePE, prwin, enhwin, prefixPr="bait", prefixEnh="oe", unique_interaction = unique_interaction)
    dfEP = niceHiCDF(hicUniqueEP, prwin, enhwin, prefixPr="oe", prefixEnh="bait", unique_interaction = unique_interaction)
...
if __name__=="__main__":
    ...
    hicTrainingCSV(hic_out_PE, hic_out_EP, prwin, enhwin, unique_interaction = "random", traindata_out = hicTraining)
